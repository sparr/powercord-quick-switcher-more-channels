...
module.exports = class Settings extends React.PureComponent {
  ...
  _setState(update) {
      const state = {
          isChannelCountValid: filterInt(this.props.getSetting('recent-channels-count')),
          initialChannelCountValue: filterInt(this.props.getSetting('recent-channels-count', 5)),
          isRecentListValid: filterArray(this.props.getSetting('recent-channels').join(",")),
          initialRecentListValue: this.props.getSetting('recent-channels', []),
          isFavoriteListValid: filterArray(this.props.getSetting('favorite-channels').join(",")),
            ...
            <TextInput
              onChange={val => {
                ...
                if (val && val>1) {
                  ...
                  updateSetting('recent-channels-count', this.state.initialChannelCountValue);
                }
              }}
              defaultValue={getSetting('recent-channels-count', 5)}
              style={!this.state.isChannelCountValid ? { borderColor: 'red' } : {}}
            ...
            <TextInput
              onChange={val => {
                ...
                if (val !== false) {
                  ...
                  updateSetting('recent-channels', this.state.initialRecentListValue);
                }
              }}
              defaultValue={getSetting('recent-channels', []).join(',')}
              style={!this.state.isRecentListValid ? { borderColor: 'red' } : {}}
            ...
            <TextInput
              onChange={val => {
                ...
                if (val !== false) {
                  ...
                  updateSetting('favorite-channels', this.state.initialFavoriteListValue);
                }
              }}
              defaultValue={getSetting('favorite-channels', []).join(',')}
              style={!this.state.isFavoriteListValid ? { borderColor: 'red' } : {}}
